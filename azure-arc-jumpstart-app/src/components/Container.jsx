/* Code generated with AutoHTML Plugin for Figma */
import "./Container.css";

export const Container = (props) => {
    return (
        <div className="breadcrumb-bar">
            <div className="rectangle-36705"></div>
            <div className="frame-63">
                <div className="breadcrumb-item">
                    <div className="breadcrumb">Breadcrumb </div>
                </div>
                <div className="">/ </div>
                <div className="breadcrumb-item">
                    <div className="breadcrumb">Breadcrumb </div>
                </div>
                <div className="">/ </div>
                <div className="breadcrumb-item">
                    <div className="breadcrumb">Breadcrumb </div>
                </div>
                <div className="">/ </div>
                <div className="breadcrumb-item">
                    <div className="breadcrumb">Breadcrumb </div>
                </div>
                <div className="">/ </div>
                <div className="breadcrumb-item">
                    <div className="breadcrumb">Breadcrumb </div>
                </div>
                <div className="">/ </div>
                <div className="breadcrumb-item">
                    <div className="breadcrumb">Breadcrumb </div>
                </div>
                <div className="">/ </div>
                <div className="breadcrumb-item">
                    <div className="breadcrumb">Breadcrumb </div>
                </div>
                <div className="">/ </div>
                <div className="breadcrumb-item">
                    <div className="breadcrumb2">Current </div>
                </div>
            </div>
            <div className="frame-60"></div>
            <div className="frame-2018775997">
                <div className="main-menu">
                    <div className="frame-59941">
                        <div className="community">Jump to section </div>
                        <svg
                            className="chevron"
                            width="15"
                            height="15"
                            viewBox="0 0 15 15"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                        >
                            <path
                                d="M3.00484 5.18721C3.23265 4.95941 3.60199 4.95941 3.8298 5.18721L7.50065 8.85807L11.1715 5.18721C11.3993 4.95941 11.7687 4.95941 11.9965 5.18721C12.2243 5.41502 12.2243 5.78436 11.9965 6.01217L7.91313 10.0955C7.68533 10.3233 7.31597 10.3233 7.08818 10.0955L3.00484 6.01217C2.77703 5.78436 2.77703 5.41502 3.00484 5.18721Z"
                                fill="white"
                            />
                        </svg>
                    </div>
                </div>
            </div>
        </div>
        // <div className="nav-bar-prepurchase-exp">
        //     <div className="breadcrumb-bar">
        //         <div className="rectangle-36705"></div>
        //         <div className="frame-63">
        //             <div className="breadcrumb-item">
        //                 <div className="breadcrumb">Breadcrumb </div>
        //             </div>
        //             <div className="">/ </div>
        //             <div className="breadcrumb-item">
        //                 <div className="breadcrumb">Breadcrumb </div>
        //             </div>
        //             <div className="">/ </div>
        //             <div className="breadcrumb-item">
        //                 <div className="breadcrumb">Breadcrumb </div>
        //             </div>
        //             <div className="">/ </div>
        //             <div className="breadcrumb-item">
        //                 <div className="breadcrumb">Breadcrumb </div>
        //             </div>
        //             <div className="">/ </div>
        //             <div className="breadcrumb-item">
        //                 <div className="breadcrumb">Breadcrumb </div>
        //             </div>
        //             <div className="">/ </div>
        //             <div className="breadcrumb-item">
        //                 <div className="breadcrumb">Breadcrumb </div>
        //             </div>
        //             <div className="">/ </div>
        //             <div className="breadcrumb-item">
        //                 <div className="breadcrumb">Breadcrumb </div>
        //             </div>
        //             <div className="">/ </div>
        //             <div className="breadcrumb-item">
        //                 <div className="breadcrumb2">Current </div>
        //             </div>
        //         </div>
        //         <div className="frame-60"></div>
        //         <div className="frame-2018775997">
        //             <div className="main-menu">
        //                 <div className="frame-59941">
        //                     <div className="community">Jump to section </div>
        //                     <svg
        //                         className="chevron"
        //                         width="15"
        //                         height="15"
        //                         viewBox="0 0 15 15"
        //                         fill="none"
        //                         xmlns="http://www.w3.org/2000/svg"
        //                     >
        //                         <path
        //                             d="M3.00484 5.18721C3.23265 4.95941 3.60199 4.95941 3.8298 5.18721L7.50065 8.85807L11.1715 5.18721C11.3993 4.95941 11.7687 4.95941 11.9965 5.18721C12.2243 5.41502 12.2243 5.78436 11.9965 6.01217L7.91313 10.0955C7.68533 10.3233 7.31597 10.3233 7.08818 10.0955L3.00484 6.01217C2.77703 5.78436 2.77703 5.41502 3.00484 5.18721Z"
        //                             fill="white"
        //                         />
        //                     </svg>
        //                 </div>
        //             </div>
        //         </div>
        //     </div>
        //     <div className="nav-bar">
        //         <div className="rectangle-148"></div>
        //         <div className="frame-67">
        //             <div className="frame-2018775958">
        //                 <div className="copilot">Copilot </div>
        //                 <svg
        //                     className="copilot2"
        //                     width="17"
        //                     height="17"
        //                     viewBox="0 0 17 17"
        //                     fill="none"
        //                     xmlns="http://www.w3.org/2000/svg"
        //                 >
        //                     <path
        //                         d="M7.12946 0.83374L7.05081 0.881308C7.05081 0.881308 8.40381 0.974473 8.84998 0.974473L9.19998 3.62464L7.44998 5.37462L5.69999 6.59099V7.9936C5.69999 8.97324 6.21199 9.88164 7.05006 10.3889L8.89235 11.5041L4.99999 13.7746H4.24926L2.85007 12.9277C2.012 12.4204 1.5 11.512 1.5 10.5323V5.81581C1.5 4.83586 2.01232 3.92722 2.85083 3.42004L7.05081 0.879587C7.07684 0.863845 7.10306 0.848563 7.12946 0.83374Z"
        //                         fill="url(#paint0_radial_3943_6628)"
        //                     />
        //                     <path
        //                         d="M7.12946 0.83374L7.05081 0.881308C7.05081 0.881308 8.40381 0.974473 8.84998 0.974473L9.19998 3.62464L7.44998 5.37462L5.69999 6.59099V7.9936C5.69999 8.97324 6.21199 9.88164 7.05006 10.3889L8.89235 11.5041L4.99999 13.7746H4.24926L2.85007 12.9277C2.012 12.4204 1.5 11.512 1.5 10.5323V5.81581C1.5 4.83586 2.01232 3.92722 2.85083 3.42004L7.05081 0.879587C7.07684 0.863845 7.10306 0.848563 7.12946 0.83374Z"
        //                         fill="url(#paint1_linear_3943_6628)"
        //                     />
        //                     <path
        //                         d="M11.2998 6.42456V7.99265C11.2998 8.97228 10.7878 9.88068 9.94972 10.388L5.74974 12.9302C4.89058 13.4503 3.82157 13.469 2.94727 12.9865L7.04988 15.4698C7.9412 16.0093 9.05838 16.0093 9.94971 15.4698L14.1497 12.9276C14.9878 12.4203 15.4998 11.5119 15.4998 10.5323V9.39951L15.1498 8.87452L11.2998 6.42456Z"
        //                         fill="url(#paint2_radial_3943_6628)"
        //                     />
        //                     <path
        //                         d="M11.2998 6.42456V7.99265C11.2998 8.97228 10.7878 9.88068 9.94972 10.388L5.74974 12.9302C4.89058 13.4503 3.82157 13.469 2.94727 12.9865L7.04988 15.4698C7.9412 16.0093 9.05838 16.0093 9.94971 15.4698L14.1497 12.9276C14.9878 12.4203 15.4998 11.5119 15.4998 10.5323V9.39951L15.1498 8.87452L11.2998 6.42456Z"
        //                         fill="url(#paint3_linear_3943_6628)"
        //                     />
        //                     <path
        //                         d="M14.1484 3.41997L9.94837 0.879526C9.08502 0.357309 8.00995 0.341131 7.13348 0.830993L7.05004 0.881461C6.21154 1.38865 5.69922 2.29728 5.69922 3.27724V6.59655L7.05004 5.77947C7.941 5.24056 9.05742 5.24056 9.94837 5.77947L14.1484 8.31992C14.9606 8.81125 15.4668 9.67931 15.4977 10.624C15.4987 10.5935 15.4992 10.5629 15.4992 10.5323V5.81575C15.4992 4.8358 14.9869 3.92716 14.1484 3.41997Z"
        //                         fill="url(#paint4_radial_3943_6628)"
        //                     />
        //                     <path
        //                         d="M14.1484 3.41997L9.94837 0.879526C9.08502 0.357309 8.00995 0.341131 7.13348 0.830993L7.05004 0.881461C6.21154 1.38865 5.69922 2.29728 5.69922 3.27724V6.59655L7.05004 5.77947C7.941 5.24056 9.05742 5.24056 9.94837 5.77947L14.1484 8.31992C14.9606 8.81125 15.4668 9.67931 15.4977 10.624C15.4987 10.5935 15.4992 10.5629 15.4992 10.5323V5.81575C15.4992 4.8358 14.9869 3.92716 14.1484 3.41997Z"
        //                         fill="url(#paint5_linear_3943_6628)"
        //                     />
        //                     <defs>
        //                         <radialGradient
        //                             id="paint0_radial_3943_6628"
        //                             cx="0"
        //                             cy="0"
        //                             r="1"
        //                             gradientUnits="userSpaceOnUse"
        //                             gradientTransform="translate(5.66486 3.27203) rotate(96.1042) scale(13.0275 15.7642)"
        //                         >
        //                             <stop offset="0.0849293" stopColor="#AE7FE2" />
        //                             <stop offset="1" stopColor="#0078D4" />
        //                         </radialGradient>
        //                         <linearGradient
        //                             id="paint1_linear_3943_6628"
        //                             x1="6.44479"
        //                             y1="12.421"
        //                             x2="5.85037"
        //                             y2="11.2535"
        //                             gradientUnits="userSpaceOnUse"
        //                         >
        //                             <stop offset="0.9999" stopColor="#114A8B" />
        //                             <stop offset="1" stopColor="#0078D4" stopOpacity="0" />
        //                         </linearGradient>
        //                         <radialGradient
        //                             id="paint2_radial_3943_6628"
        //                             cx="0"
        //                             cy="0"
        //                             r="1"
        //                             gradientUnits="userSpaceOnUse"
        //                             gradientTransform="translate(4.33177 13.1666) rotate(-20.8881) scale(10.9042 9.79964)"
        //                         >
        //                             <stop offset="0.125644" stopColor="#D59DFF" />
        //                             <stop offset="1" stopColor="#5E438F" />
        //                         </radialGradient>
        //                         <linearGradient
        //                             id="paint3_linear_3943_6628"
        //                             x1="13.3158"
        //                             y1="8.67396"
        //                             x2="12.9278"
        //                             y2="9.33815"
        //                             gradientUnits="userSpaceOnUse"
        //                         >
        //                             <stop offset="0.9999" stopColor="#493474" />
        //                             <stop offset="1" stopColor="#8C66BA" stopOpacity="0" />
        //                         </linearGradient>
        //                         <radialGradient
        //                             id="paint4_radial_3943_6628"
        //                             cx="0"
        //                             cy="0"
        //                             r="1"
        //                             gradientUnits="userSpaceOnUse"
        //                             gradientTransform="translate(14.7085 8.64459) rotate(-160.581) scale(12.774 12.8339)"
        //                         >
        //                             <stop stopColor="#50E6FF" />
        //                             <stop offset="1" stopColor="#436DCD" />
        //                         </radialGradient>
        //                         <linearGradient
        //                             id="paint5_linear_3943_6628"
        //                             x1="4.99798"
        //                             y1="4.38189"
        //                             x2="7.04037"
        //                             y2="4.38189"
        //                             gradientUnits="userSpaceOnUse"
        //                         >
        //                             <stop offset="0.9999" stopColor="#2D3F80" />
        //                             <stop offset="1" stopColor="#436DCD" stopOpacity="0" />
        //                         </linearGradient>
        //                     </defs>
        //                 </svg>
        //             </div>
        //             <div className="frame-2018775957">
        //                 <div className="git-hub">GitHub </div>
        //                 <svg
        //                     className="git-hub-1"
        //                     width="17"
        //                     height="17"
        //                     viewBox="0 0 17 17"
        //                     fill="none"
        //                     xmlns="http://www.w3.org/2000/svg"
        //                 >
        //                     <g clipPath="url(#clip0_3943_6634)">
        //                         <path
        //                             fillRule="evenodd"
        //                             clipRule="evenodd"
        //                             d="M8.5 0.175049C4.08 0.175049 0.5 3.75505 0.5 8.17505C0.5 11.715 2.79 14.705 5.97 15.765C6.37 15.835 6.52 15.595 6.52 15.385C6.52 15.195 6.51 14.565 6.51 13.895C4.5 14.265 3.98 13.405 3.82 12.955C3.73 12.725 3.34 12.015 3 11.825C2.72 11.675 2.32 11.305 2.99 11.295C3.62 11.285 4.07 11.875 4.22 12.115C4.94 13.325 6.09 12.985 6.55 12.775C6.62 12.255 6.83 11.905 7.06 11.705C5.28 11.505 3.42 10.815 3.42 7.75505C3.42 6.88505 3.73 6.16505 4.24 5.60505C4.16 5.40505 3.88 4.58505 4.32 3.48505C4.32 3.48505 4.99 3.27505 6.52 4.30505C7.16 4.12505 7.84 4.03505 8.52 4.03505C9.2 4.03505 9.88 4.12505 10.52 4.30505C12.05 3.26505 12.72 3.48505 12.72 3.48505C13.16 4.58505 12.88 5.40505 12.8 5.60505C13.31 6.16505 13.62 6.87505 13.62 7.75505C13.62 10.825 11.75 11.505 9.97 11.705C10.26 11.955 10.51 12.435 10.51 13.185C10.51 14.255 10.5 15.115 10.5 15.385C10.5 15.595 10.65 15.845 11.05 15.765C14.21 14.705 16.5 11.705 16.5 8.17505C16.5 3.75505 12.92 0.175049 8.5 0.175049Z"
        //                             fill="white"
        //                         />
        //                     </g>
        //                     <defs>
        //                         <clipPath id="clip0_3943_6634">
        //                             <rect
        //                                 width="16"
        //                                 height="16"
        //                                 fill="white"
        //                                 transform="translate(0.5 0.175049)"
        //                             />
        //                         </clipPath>
        //                     </defs>
        //                 </svg>
        //             </div>
        //             <div className="frame-20187759562">
        //                 <div className="sign-in">Sign in </div>
        //                 <svg
        //                     className="frame"
        //                     width="17"
        //                     height="17"
        //                     viewBox="0 0 17 17"
        //                     fill="none"
        //                     xmlns="http://www.w3.org/2000/svg"
        //                 >
        //                     <path
        //                         d="M12.337 9.5086C13.165 9.5086 13.8362 10.1798 13.8362 11.0079V11.3915C13.8362 11.9877 13.6232 12.5642 13.2355 13.0171C12.1892 14.2395 10.5978 14.8427 8.50092 14.8427C6.40365 14.8427 4.81298 14.2393 3.76878 13.0164C3.38226 12.5638 3.16992 11.9881 3.16992 11.3929V11.0079C3.16992 10.1798 3.84116 9.5086 4.66917 9.5086H12.337ZM12.337 10.5086H4.66917C4.39344 10.5086 4.16992 10.7321 4.16992 11.0079V11.3929C4.16992 11.7501 4.29733 12.0955 4.52923 12.367C5.36477 13.3455 6.67535 13.8427 8.50092 13.8427C10.3264 13.8427 11.6382 13.3455 12.4758 12.3669C12.7084 12.0951 12.8362 11.7491 12.8362 11.3915V11.0079C12.8362 10.7321 12.6127 10.5086 12.337 10.5086ZM8.50092 1.51172C10.3418 1.51172 11.8342 3.00411 11.8342 4.84505C11.8342 6.68601 10.3418 8.1784 8.50092 8.1784C6.65994 8.1784 5.16756 6.68601 5.16756 4.84505C5.16756 3.00411 6.65994 1.51172 8.50092 1.51172ZM8.50092 2.51172C7.21225 2.51172 6.16755 3.55639 6.16755 4.84505C6.16755 6.13372 7.21225 7.1784 8.50092 7.1784C9.78958 7.1784 10.8342 6.13372 10.8342 4.84505C10.8342 3.55639 9.78958 2.51172 8.50092 2.51172Z"
        //                         fill="white"
        //                     />
        //                 </svg>
        //             </div>
        //         </div>
        //         <div className="frame-65">
        //             <div className="menu-items">
        //                 <div className="frame-599412">
        //                     <div className="getting-started2">Getting started </div>
        //                     <svg
        //                         className="chevron2"
        //                         width="15"
        //                         height="15"
        //                         viewBox="0 0 15 15"
        //                         fill="none"
        //                         xmlns="http://www.w3.org/2000/svg"
        //                     >
        //                         <path
        //                             d="M11.9952 9.33769C11.7674 9.5655 11.398 9.5655 11.1702 9.33769L7.49935 5.66683L3.82849 9.33769C3.6007 9.5655 3.23133 9.5655 3.00354 9.33769C2.77575 9.10988 2.77575 8.74054 3.00354 8.51273L7.08687 4.4294C7.31467 4.20161 7.68403 4.20161 7.91182 4.4294L11.9952 8.51273C12.223 8.74054 12.223 9.10988 11.9952 9.33769Z"
        //                             fill="white"
        //                         />
        //                     </svg>
        //                 </div>
        //             </div>
        //             <div className="menu-items2">
        //                 <div className="frame-599412">
        //                     <div className="solutions2">Solutions </div>
        //                     <svg
        //                         className="chevron3"
        //                         width="15"
        //                         height="15"
        //                         viewBox="0 0 15 15"
        //                         fill="none"
        //                         xmlns="http://www.w3.org/2000/svg"
        //                     >
        //                         <path
        //                             d="M3.00484 5.01241C3.23265 4.7846 3.60199 4.7846 3.8298 5.01241L7.50065 8.68327L11.1715 5.01241C11.3993 4.7846 11.7687 4.7846 11.9965 5.01241C12.2243 5.24021 12.2243 5.60956 11.9965 5.83737L7.91313 9.92069C7.68533 10.1485 7.31597 10.1485 7.08818 9.92069L3.00484 5.83737C2.77703 5.60956 2.77703 5.24021 3.00484 5.01241Z"
        //                             fill="white"
        //                         />
        //                     </svg>
        //                 </div>
        //             </div>
        //             <div className="menu-items2">
        //                 <div className="frame-599412">
        //                     <div className="community">Community </div>
        //                     <svg
        //                         className="chevron4"
        //                         width="15"
        //                         height="15"
        //                         viewBox="0 0 15 15"
        //                         fill="none"
        //                         xmlns="http://www.w3.org/2000/svg"
        //                     >
        //                         <path
        //                             d="M3.00484 5.01241C3.23265 4.7846 3.60199 4.7846 3.8298 5.01241L7.50065 8.68327L11.1715 5.01241C11.3993 4.7846 11.7687 4.7846 11.9965 5.01241C12.2243 5.24021 12.2243 5.60956 11.9965 5.83737L7.91313 9.92069C7.68533 10.1485 7.31597 10.1485 7.08818 9.92069L3.00484 5.83737C2.77703 5.60956 2.77703 5.24021 3.00484 5.01241Z"
        //                             fill="white"
        //                         />
        //                     </svg>
        //                 </div>
        //             </div>
        //         </div>
        //         <div className="search-bar">
        //             <div className="search">Search </div>
        //             <div className="rectangle-36769"></div>
        //             <svg
        //                 className="icon-search"
        //                 width="18"
        //                 height="15"
        //                 viewBox="0 0 18 15"
        //                 fill="none"
        //                 xmlns="http://www.w3.org/2000/svg"
        //             >
        //                 <circle
        //                     cx="11.3285"
        //                     cy="5.92131"
        //                     r="4.5"
        //                     transform="rotate(-6.01625 11.3285 5.92131)"
        //                     stroke="white"
        //                 />
        //                 <path
        //                     d="M2.22132 14.9253L7.66422 9.32402"
        //                     stroke="white"
        //                     strokeLinecap="round"
        //                 />
        //             </svg>
        //         </div>
        //     </div> 
        //      <div className="microsoft-azure-jumpstart">
        //         <div className="jumpstart-logo">
        //             <div className="arc-jumpstart2">Arc Jumpstart </div>
        //         </div>
        //         <svg
        //             className="ms-azure-logo-horizontal-c-white-rgb"
        //             width="114"
        //             height="17"
        //             viewBox="0 0 114 17"
        //             fill="none"
        //             xmlns="http://www.w3.org/2000/svg"
        //         >
        //             <path
        //                 fillRule="evenodd"
        //                 clipRule="evenodd"
        //                 d="M83.8043 5.31116L82.3711 9.48103H85.2778L83.8516 5.31116H83.8043ZM84.8026 3.47559L88.4529 13.3762H86.6008L85.7423 10.9183H81.923L81.0984 13.3762H79.2598L82.9099 3.47559H84.8026Z"
        //                 fill="white"
        //             />
        //             <path
        //                 fillRule="evenodd"
        //                 clipRule="evenodd"
        //                 d="M89.2862 6.27783H94.836V6.94771L91.1924 12.0499H94.8562V13.3754H89.0293V12.5813L92.6188 7.60339H89.2862V6.27783Z"
        //                 fill="white"
        //             />
        //             <path
        //                 fillRule="evenodd"
        //                 clipRule="evenodd"
        //                 d="M102.058 6.27783V13.3754H100.408V12.4431H100.381C100.165 12.7656 99.8753 13.0175 99.5127 13.1992C99.1498 13.3809 98.7433 13.4721 98.2927 13.4721C97.4858 13.4721 96.8719 13.2384 96.4505 12.7711C96.0291 12.304 95.8184 11.578 95.8184 10.593V6.27783H97.4746V10.3927C97.4746 10.9911 97.5906 11.4409 97.8228 11.7425C98.0548 12.0439 98.4051 12.1947 98.8741 12.1947C99.3381 12.1947 99.7097 12.0279 99.9893 11.6942C100.269 11.3605 100.408 10.9242 100.408 10.3858V6.27783H102.058Z"
        //                 fill="white"
        //             />
        //             <path
        //                 fillRule="evenodd"
        //                 clipRule="evenodd"
        //                 d="M106.757 6.1604C106.888 6.1604 107.005 6.16957 107.109 6.1879C107.212 6.20646 107.3 6.22938 107.372 6.25711V7.94866C107.287 7.88404 107.162 7.82331 106.997 7.76555C106.833 7.70803 106.633 7.67938 106.399 7.67938C105.998 7.67938 105.659 7.85195 105.382 8.19709C105.105 8.54246 104.966 9.07392 104.966 9.79193V13.3751H103.33V6.27774H104.966V7.39635H104.993C105.142 7.0095 105.367 6.70699 105.669 6.48835C105.971 6.26995 106.334 6.1604 106.757 6.1604Z"
        //                 fill="white"
        //             />
        //             <path
        //                 fillRule="evenodd"
        //                 clipRule="evenodd"
        //                 d="M112.238 9.10884C112.238 8.55653 112.113 8.13553 111.863 7.84539C111.613 7.55548 111.26 7.41064 110.805 7.41064C110.413 7.41064 110.062 7.55457 109.751 7.84218C109.44 8.1298 109.232 8.55194 109.129 9.10884H112.238ZM113.286 11.5872V12.9614C113.016 13.1362 112.662 13.2776 112.225 13.3858C111.787 13.494 111.321 13.5481 110.825 13.5481C109.766 13.5481 108.943 13.2284 108.354 12.5885C107.767 11.9489 107.473 11.0581 107.473 9.91655C107.473 8.81673 107.787 7.91079 108.416 7.19989C109.044 6.48852 109.841 6.1333 110.805 6.1333C111.765 6.1333 112.512 6.43329 113.046 7.03396C113.58 7.63464 113.847 8.46426 113.847 9.52305V10.3309H109.088C109.16 11.0443 109.384 11.5423 109.761 11.8254C110.137 12.1086 110.623 12.25 111.217 12.25C111.609 12.25 111.984 12.1891 112.34 12.0671C112.696 11.9452 113.011 11.7853 113.286 11.5872Z"
        //                 fill="white"
        //             />
        //             <path
        //                 fillRule="evenodd"
        //                 clipRule="evenodd"
        //                 d="M72.966 7.64549H70.5296V13.3758H68.8737V7.64549H67.7109V6.27846H68.8737V5.29116C68.8737 4.54542 69.1111 3.93444 69.5868 3.45821C70.062 2.98152 70.6714 2.74341 71.4151 2.74341C71.6134 2.74341 71.7891 2.75372 71.9424 2.77458C72.0956 2.79543 72.2309 2.82637 72.3481 2.86762V4.31075C72.2938 4.27866 72.1993 4.23947 72.064 4.19341C71.9289 4.14734 71.7734 4.12443 71.5977 4.12443C71.2551 4.12443 70.9916 4.23351 70.8068 4.45215C70.6219 4.67101 70.5296 4.99438 70.5296 5.42225V6.27846H72.966V4.68339L74.6086 4.17278V6.27846H76.265V7.64549H74.6086V10.966C74.6086 11.4035 74.6864 11.712 74.8419 11.8912C74.9974 12.0709 75.2418 12.1605 75.5752 12.1605C75.6699 12.1605 75.7837 12.1378 75.9167 12.0915C76.0496 12.0457 76.1656 11.9905 76.265 11.9261V13.3066C76.1611 13.3667 75.9888 13.4217 75.7475 13.4725C75.5066 13.523 75.2689 13.5484 75.0344 13.5484C74.3449 13.5484 73.8279 13.3609 73.4832 12.9858C73.1384 12.6106 72.966 12.0457 72.966 11.2905V7.64549ZM62.1015 9.87467C62.1015 10.6158 62.266 11.1819 62.5949 11.5733C62.9237 11.9645 63.3946 12.1598 64.0077 12.1598C64.6025 12.1598 65.0556 11.9645 65.3663 11.5733C65.6773 11.1819 65.8328 10.6018 65.8328 9.83319C65.8328 9.06934 65.6717 8.49273 65.3495 8.10382C65.0273 7.71491 64.5754 7.52057 63.9942 7.52057C63.3946 7.52057 62.9295 7.72407 62.5983 8.13155C62.2669 8.53857 62.1015 9.11999 62.1015 9.87467ZM60.4003 9.92942C60.4003 8.75581 60.7248 7.82604 61.3737 7.14012C62.0227 6.45442 62.924 6.11157 64.0776 6.11157C65.1636 6.11157 66.012 6.44182 66.6228 7.1023C67.2334 7.76302 67.5386 8.65452 67.5386 9.77771C67.5386 10.9284 67.2141 11.8442 66.5654 12.5253C65.9162 13.2066 65.033 13.5472 63.9154 13.5472C62.8383 13.5472 61.9832 13.2241 61.3502 12.5771C60.7169 11.9306 60.4003 11.048 60.4003 9.92942ZM56.7148 8.14237C56.7148 8.38163 56.7893 8.5691 56.938 8.705C57.0866 8.84067 57.4155 9.0121 57.9249 9.21927C58.5783 9.48626 59.037 9.78671 59.3004 10.1204C59.564 10.4541 59.696 10.8579 59.696 11.3321C59.696 11.9994 59.4447 12.5357 58.9423 12.9404C58.4396 13.3458 57.7604 13.5482 56.9042 13.5482C56.6158 13.5482 56.2967 13.5127 55.9476 13.4412C55.5982 13.3699 55.302 13.2789 55.0586 13.1684V11.5255C55.3561 11.737 55.6761 11.905 56.0185 12.0292C56.3611 12.1536 56.6719 12.2158 56.9515 12.2158C57.3208 12.2158 57.5937 12.1628 57.7694 12.0569C57.9451 11.9511 58.0331 11.7739 58.0331 11.5255C58.0331 11.2952 57.9417 11.1006 57.7593 10.942C57.5769 10.7829 57.2309 10.6001 56.7215 10.3929C56.1177 10.1353 55.6898 9.84538 55.4373 9.52316C55.1849 9.20094 55.0586 8.7914 55.0586 8.29409C55.0586 7.65446 55.3076 7.1285 55.8056 6.71644C56.3035 6.30461 56.9493 6.09881 57.7425 6.09881C57.9857 6.09881 58.2583 6.12631 58.5604 6.18131C58.8622 6.23677 59.1146 6.30827 59.3174 6.39559V7.98355C59.1011 7.83619 58.8487 7.70969 58.5604 7.60358C58.2718 7.4977 57.9857 7.44499 57.7019 7.44499C57.3909 7.44499 57.1485 7.50709 56.9751 7.63131C56.8016 7.75552 56.7148 7.92603 56.7148 8.14237ZM48.8476 9.87467C48.8476 10.6158 49.0119 11.1819 49.341 11.5733C49.67 11.9645 50.141 12.1598 50.7538 12.1598C51.3489 12.1598 51.8017 11.9645 52.1125 11.5733C52.4235 11.1819 52.579 10.6018 52.579 9.83319C52.579 9.06934 52.4178 8.49273 52.0956 8.10382C51.7734 7.71491 51.3215 7.52057 50.7403 7.52057C50.141 7.52057 49.6756 7.72407 49.3444 8.13155C49.013 8.53857 48.8476 9.11999 48.8476 9.87467ZM47.1445 9.92942C47.1445 8.75581 47.4689 7.82604 48.1179 7.14012C48.7668 6.45442 49.6682 6.11157 50.8217 6.11157C51.908 6.11157 52.7562 6.44182 53.367 7.1023C53.9775 7.76302 54.2829 8.65452 54.2829 9.77771C54.2829 10.9284 53.9582 11.8442 53.3095 12.5253C52.6606 13.2066 51.7772 13.5472 50.6595 13.5472C49.5824 13.5472 48.7273 13.2241 48.0943 12.5771C47.4611 11.9306 47.1445 11.048 47.1445 9.92942ZM46.4409 6.1604C46.5715 6.1604 46.6889 6.1698 46.7923 6.18836C46.896 6.20669 46.9839 6.22938 47.0562 6.25711V7.94866C46.9705 7.88427 46.8453 7.82331 46.681 7.76578C46.5163 7.70803 46.3168 7.67938 46.0828 7.67938C45.6816 7.67938 45.3425 7.85195 45.0654 8.19709C44.7883 8.54246 44.6496 9.07392 44.6496 9.79193V13.3753H43.0136V6.27797H44.6496V7.39635H44.6766C44.8253 7.00973 45.0506 6.70722 45.3526 6.48835C45.6544 6.26995 46.0173 6.1604 46.4409 6.1604ZM40.1669 12.1602C40.4101 12.1602 40.6783 12.103 40.9711 11.9879C41.2642 11.8726 41.5345 11.7209 41.7825 11.5321V13.0854C41.5211 13.2374 41.2247 13.3524 40.8935 13.4306C40.5625 13.5089 40.1983 13.5481 39.8018 13.5481C38.7786 13.5481 37.9473 13.2179 37.3075 12.5574C36.6674 11.8969 36.3476 11.0536 36.3476 10.0273C36.3476 8.88554 36.6743 7.94569 37.3277 7.20659C37.9811 6.46796 38.9072 6.09875 40.1061 6.09875C40.4126 6.09875 40.7223 6.13909 41.0355 6.21953C41.3487 6.29997 41.5976 6.39302 41.7825 6.4989V8.10084C41.5301 7.912 41.272 7.76601 41.0084 7.6622C40.7449 7.55884 40.4754 7.50704 40.2006 7.50704C39.5561 7.50704 39.0358 7.7211 38.639 8.1492C38.2426 8.57707 38.0442 9.15482 38.0442 9.882C38.0442 10.6 38.2345 11.1594 38.6155 11.5598C38.9963 11.9602 39.5135 12.1602 40.1669 12.1602ZM33.5703 13.3748H35.206V6.27747H33.5703V13.3748ZM33.4004 4.22672C33.4004 3.95056 33.4984 3.71909 33.6946 3.53277C33.8905 3.34622 34.1236 3.25317 34.394 3.25317C34.6823 3.25317 34.9215 3.34874 35.1107 3.53964C35.3001 3.73055 35.3945 3.95973 35.3945 4.22672C35.3945 4.49806 35.2976 4.72609 35.1039 4.90989C34.9101 5.09415 34.6736 5.18605 34.394 5.18605C34.1146 5.18605 33.8792 5.093 33.6878 4.90668C33.496 4.72013 33.4004 4.49348 33.4004 4.22672ZM31.9932 3.47518V13.3756H30.3101V5.61546H30.2829L27.2748 13.3756H26.1596L23.0769 5.61546H23.0567V13.3756H21.502V3.47518H23.9152L26.7003 10.8141H26.7407L29.6813 3.47518H31.9932Z"
        //                 fill="white"
        //             />
        //             <path
        //                 fillRule="evenodd"
        //                 clipRule="evenodd"
        //                 d="M0.501953 8.02041H8.18027V0.178223H0.501953V8.02041Z"
        //                 fill="#F25022"
        //             />
        //             <path
        //                 fillRule="evenodd"
        //                 clipRule="evenodd"
        //                 d="M8.97656 8.01724H16.6549V0.175049H8.97656V8.01724Z"
        //                 fill="#7FBA00"
        //             />
        //             <path
        //                 fillRule="evenodd"
        //                 clipRule="evenodd"
        //                 d="M0.5 16.6747H8.17832V8.83252H0.5V16.6747Z"
        //                 fill="#00A4EF"
        //             />
        //             <path
        //                 fillRule="evenodd"
        //                 clipRule="evenodd"
        //                 d="M8.97656 16.6747H16.6549V8.83252H8.97656V16.6747Z"
        //                 fill="#FFB900"
        //             />
        //         </svg>

        //         <svg
        //             className="line-123"
        //             height="23"
        //             viewBox="0 0 0 23"
        //             fill="none"
        //             xmlns="http://www.w3.org/2000/svg"
        //         >
        //             <path d="M0 0L0 23" stroke="white" strokeLinecap="round" />
        //         </svg>
        //     </div> 
        //      <div className="getting-started3">
        //         <div className="frame-20187759842">
        //             <div className="sub-menu3">
        //                 <div className="tab-line"></div>
        //                 <div className="sub-menu-body">
        //                     <div className="sub-menu-items2">
        //                         <div className="overview4">Overview </div>
        //                     </div>
        //                     <div className="sub-menu-items2">
        //                         <div className="overview2">Jumpstart Scenarios </div>
        //                     </div>
        //                     <div className="sub-menu-items2">
        //                         <div className="overview2">Additional resources </div>
        //                     </div>
        //                     <div className="sub-menu-items2">
        //                         <div className="overview2">About </div>
        //                     </div>
        //                 </div>
        //             </div>
        //             <div className="frame-2018775955">
        //                 <div className="frame-2018775954">
        //                     <div className="frame-59895">
        //                         <div className="overview5">Overview </div>
        //                         <div className="view-all2">View all </div>
        //                     </div>
        //                     <div className="description2">
        //                         The goal of the repo is for you to have a working Azure Arc demo
        //                         environment spun up in no-time so you can focus on playing,
        //                         demoing, upskilling yourself and your team and see the core
        //                         values of Azure Arc. Explore our different Jumpstart products.{" "}
        //                     </div>
        //                 </div>
        //                 <div className="frame-59653">
        //                     <div className="row-1">
        //                         <div className="nav-bar-cards-dark-mode">
        //                             <div className="card3">
        //                                 <div className="header">
        //                                     <div className="text">
        //                                         <div className="object-counting2">
        //                                             Jumpstart Solutions{" "}
        //                                         </div>
        //                                     </div>
        //                                 </div>
        //                                 <div className="detect-and-count-objects-or-people2">
        //                                     Accelerate your deployment with our Jumpstart solutions.{" "}
        //                                 </div>
        //                             </div>
        //                         </div>
        //                         <div className="nav-bar-cards-dark-mode">
        //                             <div className="card3">
        //                                 <div className="header">
        //                                     <div className="text">
        //                                         <div className="object-counting2">Jumpstart Agora </div>
        //                                     </div>
        //                                 </div>
        //                                 <div className="detect-and-count-objects-or-people2">
        //                                     Collection of industry verticals from cloud to edge
        //                                     scenarios.{" "}
        //                                 </div>
        //                             </div>
        //                         </div>
        //                         <div className="nav-bar-cards-dark-mode">
        //                             <div className="card3">
        //                                 <div className="header">
        //                                     <div className="text">
        //                                         <div className="object-counting2">
        //                                             Jumpstart ArcBox{" "}
        //                                         </div>
        //                                     </div>
        //                                 </div>
        //                                 <div className="detect-and-count-objects-or-people2">
        //                                     Explore Azure Arc capabilities with Azure Sandbox.{" "}
        //                                 </div>
        //                             </div>
        //                         </div>
        //                         <div className="nav-bar-cards-dark-mode">
        //                             <div className="card3">
        //                                 <div className="header">
        //                                     <div className="text">
        //                                         <div className="object-counting2">
        //                                             Jumpstart HCIBox{" "}
        //                                         </div>
        //                                     </div>
        //                                 </div>
        //                                 <div className="detect-and-count-objects-or-people2">
        //                                     Explore Azure Stack HCI capabilities with Azure Sandbox.{" "}
        //                                 </div>
        //                             </div>
        //                         </div>
        //                         <div className="nav-bar-cards-dark-mode">
        //                             <div className="card3">
        //                                 <div className="header">
        //                                     <div className="text">
        //                                         <div className="object-counting2">
        //                                             Jumpstart Lightning{" "}
        //                                         </div>
        //                                     </div>
        //                                 </div>
        //                                 <div className="detect-and-count-objects-or-people2">
        //                                     Watch our show where people share their stories.{" "}
        //                                 </div>
        //                             </div>
        //                         </div>
        //                     </div>
        //                 </div>
        //             </div>
        //         </div>
        //     </div> 
        // </div>
    );
};
